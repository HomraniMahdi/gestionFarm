...
////////////////////////////////////////////////////////////
void supprimer_capteur(char id_capteur[70])
{
	capteur c1;
	FILE *f,*g;
	f=fopen("capteur.txt","r");
...
///////////////////////////////////////////////////////////////////
void modifier_capteur(char id_capteur[70])
{
capteur c1;
	FILE *f;
	FILE *f2;
f=fopen("capteur.txt","r");
f2=fopen("capteuran.txt","w+"); 
	if (f==NULL)
	return;
	else
	{
	...
	{ 
	  while (fscanf(f,"%s %s %s %s %s %d %d %d %d %d %d\n",c1.id_capteur,c1.marque,c1.type,c1.etat,c1.valeur,&c1.datef.jour,&c1.datef.mois,&c1.datef.annee,&c1.datep.jour,&c1.datep.mois,&c1.datep.annee)!=EOF);	
		{
		 if(strcmp(id_capteur,c1.id_capteur)==0) 
			{ fprintf(f2,"%s %s %s %s %s %d %d %d %d %d %d\n",c1.id_capteur,c1.marque,c1.type,c1.etat,c1.valeur,c1.datef.jour,c1.datef.mois,c1.datef.annee,c1.datep.jour,c1.datep.mois,c1.datep.annee)
	; }
		 else   { fprintf(f2,"%s %s %s %s %s %d %d %d %d %d %d\n",c1.id_capteur,c1.marque,c1.type,c1.etat,c1.valeur,c1.datef.jour,c1.datef.mois,c1.datef.annee,c1.datep.jour,c1.datep.mois,c1.datep.annee)
	; }
	}
fclose(f2);
fclose(f);
remove("capteur.txt");
rename("capteuran.txt","capteur.txt");
	}
}
}
////////////////////////////////////////////////////////////////////
